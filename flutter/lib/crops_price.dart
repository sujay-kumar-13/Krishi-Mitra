...
class CropsPrice extends StatefulWidget {
  ...
  const CropsPrice({super.key, required this.selectedState, required this.language});
...
class _CropsPriceState extends State<CropsPrice> {
  ...
  List<Map<String, dynamic>> processCropsData(
    ...
    for (final crop in cropsData) {
      ...
      final change =
          (nextMonthPrice - previousMonthPrice) / previousMonthPrice * 100;
  ...
  Future<void> _fetchCropData() async {
    ...
    try {
      ...
      //   print(crop);
      // }
      // if(cropsCollectionData.isEmpty) print("List is empty");
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          getTranslatedValue("Crop Price", language!) ?? 'Crop Price',
          // AppLocalizations.of(context)?.cropPrice ?? 'Crops Price',
          // "Crops Prices",
            ...
            Row(
              ...
              children: [
                Text(
                  ...
                  // AppLocalizations.of(context)?.cropsIn ?? 'Crops in ',
                  // "Crops in ",
                  style: TextStyle(
                    fontSize: 24,
                          ...
                          return GridView.builder(
                            ...
                            gridDelegate:
                                SliverGridDelegateWithFixedCrossAxisCount(
                                  ...
                                  childAspectRatio:
                                      aspectRatio, // Adjusted aspect ratio
                                ),
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            // opens the price trend page on click of the card
            builder:
                (context) =>
                    PriceTrendPage(selectedState: state, cropName: name, language: language),
          ),
        );
      },
      child: Card(
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  SizedBox(height: 4),
                  Text(
                    "${getTranslatedValue("This Month", language) ?? 'This Month'} : ₹ $previousMonthPrice",
                    style: TextStyle(fontSize: 14, color: Colors.black),
                  ),
                  Text(
                    "${getTranslatedValue("Next Month", language) ?? 'Next Month'} : ₹ $nextMonthPrice",
                    style: TextStyle(fontSize: 14, color: Colors.black),
                  ),
                ],
              ),
